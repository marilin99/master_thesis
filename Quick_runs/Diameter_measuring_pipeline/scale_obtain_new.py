...
MODEL_PATH = "/home/marilin/fibar_tool/Quick_runs/Diameter_measuring_pipeline/number_models/"
UNIT_PATH = "/home/marilin/fibar_tool/Quick_runs/Diameter_measuring_pipeline/unit_models/"
...
def scale_obtain(file):
    ...
    units = ["um", "um", "um", "um", "um", "nm", "um", "um"]

    f = os.path.basename(file).split(".")[0]

    with open(f"{MODEL_PATH}{f}.npy", "wb") as np_f:
        np.save(np_f, number)

    with open(f"{UNIT_PATH}{units[FILES.index(os.path.basename(file))]}.npy", "wb") as unit_f:
        np.save(unit_f, unit)    
...
ORIG_PATH = "/home/marilin/fibar_tool/Data/Model_images/Detectable/"
FILES = os.listdir("/home/marilin/fibar_tool/Data/Model_images/Detectable/")
